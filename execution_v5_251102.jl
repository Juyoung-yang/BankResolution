...
@time Rl_star_false = find_zero(solve_model_given_r_single_false,Rl_safe,method = Roots.Secant();tol = 1e-2,maxevals = 100) # Rl = 1.65/ E = 270
@time Rl_star_false = find_zero(solve_model_given_r_single_false,Rl_safe,method = Roots.Secant();tol = 1e-2,maxevals = 100) # Rl = 1.65/ E = 263
E = 230.0;
@time Rl_star_false = find_zero(solve_model_given_r_single_false,Rl_safe,method = Roots.Secant();tol = 1e-2,maxevals = 100) # Rl = 1.65/ E = 263
E = 240.0
@time Rl_star_false = find_zero(solve_model_given_r_single_false,Rl_safe,method = Roots.Secant();tol = 1e-2,maxevals = 100) # Rl = 1.65/ E = 263
...
@time policy_false = Get_PolicyFuncs(params_ex, Iterobj_is_false, vFuncs_false, Rl_star_false, false);

@time simulation_false = simulate_and_moments(params_ex, params_cal, vFuncs_false, policy_false, Rl_star_false, false);

@show simulation_false.moments


vFuncs_false_ver2       = Initiate_vFunc(params_ex);
vFuncsNew_false_ver2   = Initiate_vFuncNew(params_ex);
Iterobj_is_false_ver2   = Initiate_MatrixIterObj_i(params_ex);

solve_model_given_r_single_false = Rl -> solve_model_given_r!(Rl;Params = params_ex,Regime = false,vFuncs = vFuncs_false_ver2,vFuncsNew = vFuncsNew_false_ver2,Iterobj_is = Iterobj_is_false_ver2)
@time Rl_star_false_ver2 = find_zero(solve_model_given_r_single_false,Rl_safe,method = Roots.Secant();tol = 1e-2,maxevals = 100) # Rl = 1.65/ E = 263
...
### 최종 parameter in Nov 1st: sigHat = 1.15; lconstr = 2.4; E = 210.0 with l_stop = 100.0
...
Rl_safe = clamp(params_cal.a, 1.0 + eps(), 1.5 - eps())
# Rl_safe = clamp(params_cal.a, 1.0 + eps(), 2.0 - eps())
...
@time VFI!(params_ex, vFuncs_true, vFuncsNew_true, Iterobj_is_true, 1.04000, true, 1000, 1e-4);
@time stationary_distribution!(params_ex, 1.04000, vFuncs_true, vFuncsNew_true, Iterobj_is_true, 1000, 1e-4);

@time policy_true = Get_PolicyFuncs(params_ex, Iterobj_is_true, vFuncs_true, 1.04000, true);

@time simulation_true = simulate_and_moments(params_ex, params_cal, vFuncs_true, policy_true, 1.04000, true);

@show simulation_true.moments

@show vFuncs_true.qBond

@show aggre_loan_supply2(params_ex, vFuncs_true, Iterobj_is_true)
@show params_ex.E * 1.040000^params_ex.ϵ
