...
policy_ex = Get_PolicyFuncs(params, sol.eq.Iterobj_is, sol.Rl_star); # get policy functions from the solution

# 2. generate shock sequenc, simulate paths, and calculate moments
shocks = makeShockSequence(params, sol.eq.vFuncs, 100, 1000, 10); # make shock sequence for simulation
typeof(shocks) # check the type of shocks
paths = Initiate_Paths(shocks, params); # initiate paths for simulation



paths_ex = Simulate_paths(paths, policy_ex, sol.eq.vFuncs, params, sol.Rl_star, 10); # simulate paths and calculate moments
