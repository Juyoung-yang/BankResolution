...
@time ex = solve_simulate_and_moments_reuse(params_ex, params_cal, true)
@show ex.moments
...
fail_temp_buffer = make_buffers(Float64) 
q_temp_buffer = make_buffers(Float64) 


include("main_v3_Reuse_dampning.jl");
qBond_condiState!(params_ex, ex.Rl_star, 2, 3, 2, 1, fail_temp_buffer[1], q_temp_buffer[1], βF)
It_example = Initiate_MatrixIterObj_i(params_ex)
vFuncsNew;
VFI!(params_ex, ex.vFuncs, vFuncsNew, It_example, ex.Rl_star, true, 100, 1e-4)
@show ex.vFuncs.qBond
...
@show params_ex.F[3, :], params_ex.cM, params_ex.cO, params_ex.cL, params_ex.E, params_ex.dBar
@show params_ex.λL
params_cal = Initiate_Params_cal(bigT,bigN,bigJ,trim,a,b,debt_to_liability,capital_to_deposit,loan_to_asset,loan_rate,cM,cO,cL,ϵ,E,dBar,σ);
@time ex = solve_simulate_and_moments_reuse(params_ex, params_cal, false);
ex.moments

@time ex_true = solve_simulate_and_moments_reuse(params_ex, params_cal, true);
ex.moments
...
@time vf = VFI(params_ex, )


include("main_v2_noLoanConstraint_withCCrule.jl");
@time ex = solve_simulate_and_moments(params_ex, params_cal, false);
@show ex.moments
